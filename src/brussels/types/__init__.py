...
try:
    from .remote_file import RemoteFile, RemoteStorage, UploadStatus
except ModuleNotFoundError as exc:
    if exc.name != "pydantic":
        raise

    def __getattr__(name: str) -> object:
        if name in {"RemoteFile", "RemoteStorage", "UploadStatus"}:
    ...
    __all__ += ["RemoteFile", "RemoteStorage", "UploadStatus"]
